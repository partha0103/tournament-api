...
function updateTournament(u_id,t_name, t_status, t_id, players, cb){
    getTstatus(t_id, function(status){
        if(status){
            ...
            getAllPlayers(u_id, function(existing_player){
                areValidPlayers(existing_player, players, function(flag){
                    if(flag){
...
function getAllPlayers(u_id, cb){
    ...
    let stmt = "select id from player where user_id = ?";
