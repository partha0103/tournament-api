        ...
        if(status){
            ...
            getAllPlayers(u_id, function(existing_player){
                if(add_players.length > 0){
                    areValidPlayers(existing_player, add_players, function(flag){
                        if(flag){
                            add_players.forEach( (player) => {
                                // registerPlayer(u_id, t_id, player);
                            })
                        }
                        else{
                    ...
                    areValidPlayers(existing_player, remove_players, function(flag){
                        if(flag){
                            remove_players.forEach( (player)=>{
                                deletePlayer(player, (message) => {
                                    console.log(message);
                                })
                            } );
                            cb("Successfully deleted all the players");
                        }
                        else{
                            cb("Added players are not valid users");
                        }
