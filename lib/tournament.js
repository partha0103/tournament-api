...
function registerPlayer(user_id, tournament_id, name,connection, cb){
...
function deleteMatches(tournament_id,connection,cb){
...
function deletePlayer(id,connection,cb){
...
function getTstatus(tournament_id,connection, cb){
...
function getTournamentName(id, connection, cb){
    let stmt = 'select name from tournament where id = ?';
    connection.query(stmt,id,function(error, results){
        if(error){
            return connection.rollback(function() {
                throw error;
...
function updateTournamentName(id, name, connection, cb){
    let stmt = "update tournament set name = ? where id = ?";
    connection.query(stmt,[name, id],function(error, results){
        if(error){
            return connection.rollback(function() {
                throw error;
            });
            ...
            cb("Updated the tournament name");
        }
    });
}
...
function updateTournament(u_id,t_name, t_status, t_id, add_players, remove_players, cb){
    ...
    connection.beginTransaction( (error) => {
        if(error){
            ...
            getTournamentName(t_id, connection,function(tournament_name){
                if(tournament_name !== t_name){
                    updateTournamentName(t_id, t_name, connection, function(updated_message){
                    ...
                    getTstatus(t_id, connection,function(status){
                        if(status){
                            ...
                            getAllPlayers(u_id, connection,function(existing_players){
                                existing_players = getPlayerIds(existing_players);
                                if(add_players.length > 0){
                                    let add_player = getPlayerIds(add_players);
                                    areValidPlayers(existing_players, add_player, function(flag){
                                        if(flag){
                                            add_players.forEach( (player,index, players) => {
                                                registerPlayer(u_id, t_id, player.name, connection, (result) => {
                                    ...
                                    console.log(remove_players);
                                    let remove_player = getPlayerIds(remove_players);
                                    areValidPlayers(existing_players, remove_player, function(flag){
                                        if(flag){
                                            remove_player.forEach( (player, index, players)=>{
                                                deletePlayer(player,connection, () => {
                                                    if(index == players.length - 1){
                                                        connection.commit( ( error )=> {
                                                            if(error){
                                                                return connection.rollback(function() {
...
function getAllPlayers(u_id,connection, cb){
